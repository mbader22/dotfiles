  (define (batch-surface-sharpen-facebook pattern)
...
(outfile (string-append filename ".png"))
  ...
  (if (> wval hval)            ; check if verti
            (gimp-image-scale-full image 2048 1360 INTERPOLATION-LANCZOS)
       )
   (if (< wval hval)            ; check if hori
            (gimp-image-scale-full image 1360 2048 INTERPOLATION-LANCZOS)
       )
	   (plug-in-sharpen RUN-NONINTERACTIVE
                                   image drawable 20)
