  (define (batch-homepage3-sharpen pattern maxwidth maxheight widthm heightm widths heigts)
    (let* ( (filelistl (cadr (file-glob pattern 1)))
      ...
      (while (not (null? filelistl))
             (let* (	(filename (car filelistl))
                    ...
                    (newfilename (string-append "phoca_t_l_" filename))
                    )
               ...
               (if (< (/ wval hval) (/ maxwidth maxheight))
                   ...
                   (begin
                     (gimp-image-scale-full image (* maxwidth (/ wval hval)) maxwidth INTERPOLATION-LANCZOS)
                     )
                   (begin
               ...
               (file-jpeg-save RUN-NONINTERACTIVE
                               image drawable (string-append "Kategoriename/thumbs/" newfilename) (string-append "thumbs/" newfilename) 1 0 1 0 "" 2 0 0 2)
               (file-jpeg-save RUN-NONINTERACTIVE
                               image drawable (string-append "Kategoriename/" filename) (string-append "thumbs/" newfilename) 1 0 1 0 "" 2 0 0 2)
